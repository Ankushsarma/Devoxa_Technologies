...
export function Chat({ chatId, receiverId, className, title, subtitle }: { chatId: string, receiverId?: string, className?: string, title?: string, subtitle?: string }) {
    ...
    return (
        <div className={`flex flex-col w-full overflow-hidden bg-theme-50 rounded-2xl border border-neutral-200 shadow-sm relative ${className || 'h-[600px]'}`}>
            
            {/* Header */}
            <div className="p-5 border-b border-neutral-200 bg-theme-50 font-sans flex justify-between items-start shrink-0">
            ...
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-5 space-y-4 relative bg-theme-50">
                {messages.length === 0 ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-theme-50">
                        ...
                        {messages.map((msg) => {
                            ...
                            return (
                                <div key={msg._id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                                    <div className={`max-w-[85%] px-4 py-2.5 text-sm font-sans rounded-2xl shadow-sm ${isMe ? "bg-[#1A1A1A] text-[#f1eef1] rounded-tr-sm" : "bg-neutral-50 text-neutral-900 border border-neutral-100 rounded-tl-sm"}`}>
                                        {!isMe && (
            ...
            {/* Input Form */}
            <form onSubmit={sendMessage} className="p-4 border-t border-neutral-200 bg-theme-50 shrink-0">
                <div className="relative flex items-center">
                    <input
                        ...
                        className="w-full h-12 bg-theme-50 border border-neutral-200 focus:border-[#C5A880] outline-none text-sm font-sans placeholder-neutral-400 rounded-xl pl-4 pr-24 transition-colors"
                    />
                    <div className="absolute right-2 flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="w-8 h-8 flex items-center justify-center bg-[#C5A880] hover:bg-[#B0926A] text-[#f1eef1] rounded-lg transition-colors disabled:opacity-50 disabled:hover:bg-[#C5A880]"
                        >
