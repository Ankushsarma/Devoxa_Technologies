...
const FAQAccordionItem = ({ faq, isOpen, onClick }) => {
  return (
    <div className="border-b border-[rgba(255,255,255,0.08)] group">
      ...
      >
        <span 
          className={`text-2xl font-serif transition-colors duration-300 ${isOpen ? 'text-[#8b5cf6]' : 'text-white group-hover:text-gray-300'}`}
          style={{ lineHeight: 1 }}
        >
          {faq.question}
        </span>
        <span className="relative flex items-center justify-center w-8 h-8 rounded-full bg-white/5 group-hover:bg-white/10 transition-colors duration-300 flex-shrink-0 ml-4">
          <Plus className={`absolute w-5 h-5 text-white transition-all duration-500 ease-[cubic-bezier(0.87,0,0.13,1)] ${isOpen ? 'rotate-180 opacity-0 scale-50' : 'rotate-0 opacity-100 scale-100'}`} />
          <X className={`absolute w-5 h-5 text-[#8b5cf6] transition-all duration-500 ease-[cubic-bezier(0.87,0,0.13,1)] ${isOpen ? 'rotate-0 opacity-100 scale-100' : '-rotate-180 opacity-0 scale-50'}`} />
        </span>
      </button>
      <div 
        ...
        <div className="overflow-hidden">
          <div className="pb-8 md:pb-10">
            <div className="flex gap-4 md:gap-5">
              {/* Line Sidebar Effect */}
              <div 
                className="w-[3px] rounded-full shrink-0 my-2.5 transition-all duration-500 ease-out bg-gradient-to-b from-[#4F46E5] to-[#7C3AED] shadow-[0_0_12px_rgba(139,92,246,0.5)]"
              />
              <p className="text-[#8981A6] font-light leading-relaxed max-w-xl text-lg pr-12 m-0">
                {faq.answer}
              </p>
            </div>
          </div>
